...
This directory contains the architecture overview and four operator-performance
...
- TorchInductor benchmarks eligible candidates from the enabled GEMM backends
  and selects the fastest measured implementation. Enabling FlyDSL does not
  force its selection.
...
dense GEMM, grouped GEMM, RMSNorm, and TopK charts, and prints the calculated
performance aggregates. SVG text remains editable. All five figures are used
in the article.
